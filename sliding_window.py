...
def sliding_window(binary_warped):
    ...
    histogram_smoother = signal.medfilt(histogram, 7)
    # Create an output image to draw on and  visualize the result
    out_img = np.dstack((binary_warped, binary_warped, binary_warped)) * 255
    # Find the peak of the left and right halves of the histogram
    # These will be the starting point for the left and right lines
    midpoint = np.int(histogram_smoother.shape[0] / 2)
    leftx_base = np.argmax(histogram_smoother[:midpoint])
    rightx_base = np.argmax(histogram_smoother[midpoint:]) + midpoint
    ...
    if ((left_fitx[0] > right_fitx[0]) | (abs(left_fitx[0] - right_fitx[0]) < 350) | (
                abs(left_fitx[0] - right_fitx[0]) > 700) |
            (left_fitx[0] > 800)):
