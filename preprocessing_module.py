...
# used for the white lanes histogram equalisation threshold
def equlize_thr(gray, thr=250):
...
def thresholding(img, grad_thx_min=211, grad_thx_max=255, grad_thy_min=0, grad_thy_max=25, mag_th_min=150,
    ...
    s_binary = color_thr(s_channel, v_channel, s_threshold=(s_threshold_min, s_threshold_max),
                         v_threshold=(v_threshold_min, v_threshold_max)).astype(np.uint8)

    # histogram equalised threshold for white lanes
    gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY).astype(np.uint8)
    eq_thr = equlize_thr(gray, adp_thr) / 255
    # Stack each channel
    # Combine binary thresholds
    # color_binary = np.dstack(( np.zeros_like(s_binary),combined, s_binary))
    color_binary = np.zeros_like(gradx)
    color_binary[(combined == 1) | (s_binary == 1) | (eq_thr == 1)] = 1
